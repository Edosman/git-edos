...
public class Graph {
    ...
    Queue<String> findShortestWayBfs(String startLabel, String finishLabel) {
        Vertex vertex = findLabel(startLabel);
        if (vertex == null) {
            throw new IllegalArgumentException(startLabel + "Такого города нет в списке!");
        }
        Queue<Vertex> queue = new ArrayDeque();
    ...
    private Vertex findLabel(String startLabel) {
        for (int i = 0; i < size; i++) {
    ...
    private Queue<String> shortestWay(Vertex vertex) {
